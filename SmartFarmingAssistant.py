...
def load_sample_data():
    try:
        ...
        return pd.DataFrame({
            ...
            'moisture': np.random.uniform(20, 300, N),
...
def load_mock_data():
    return pd.DataFrame({
        ...
        'moisture': np.random.uniform(20, 300, 100),
        'label': np.random.choice(['rice', 'wheat', 'maize', 'potato'], 100)
    })
...
def main():
    ...
    st.write("Optimize your farming decisions with AI-powered recommendations")
    ...
    with col2:
        ...
        rainfall = st.slider('Rainfall (mm)', 20.0, 300.0, weather['rainfall'])
        moisture = st.slider('Soil Moisture (%)', 10, 60, 35)
    
    # Prediction
    if st.button("Get Recommendations", type="primary"):
        input_data = [[N, P, K, ph, rainfall, moisture, weather['temperature'], weather['humidity']]]
        
        # Get predictions
        crop_num = rf_class.predict(input_data)[0]
        crop_name = le.inverse_transform([crop_num])[0]
        temp_pred, humid_pred = rf_reg.predict(input_data)[0]
        ...
        fig, ax = plt.subplots()
        sns.barplot(x=probs[top_indices], y=le.inverse_transform(top_indices), palette="viridis")
        st.pyplot(fig)
    ...
    selected_feature = st.selectbox('Select feature to visualize', ['N', 'P', 'K', 'ph', 'rainfall' ,'moisture'])
